...
GlFramebuffer::GlFramebuffer(Framebuffer::Spec& spec)
{
    ...
    {
        NB_CORE_ASSERT(false,
                       "Attempted to make null sized Framebuffer or Yuge (> %i) "
                       "Framebuffer\n",
                       Framebuffer::k_maxDimension);
    }

    NB_CORE_ASSERT(m_spec.samples, "Must have at least 1 sample ");
...
void GlFramebuffer::resize(u32_t width, u32_t height)
{
    NB_PROFILE();
...
void GlFramebuffer::blit(ref<Framebuffer> p_destination,
                         const u32_t      srcAttachmentIdx,
                         const u32_t      destAttachmentIdx) const
{
    NB_PROFILE();

    NB_CORE_ASSERT(m_fbo != p_destination->getId(), "Cannot blit to self!");
...
void GlFramebuffer::bind(Mode mode) const
{
    ...
    u32_t id     = m_fbo;
    u32_t width  = m_spec.width;
    u32_t height = m_spec.height;
...
void GlFramebuffer::bindTexture(const u32_t textureUnit, const u32_t attachmentIdx) const
{
...
void GlFramebuffer::unbindTexture(const u32_t attachmentIdx) const
{
    if (attachmentIdx >= m_colorAttachments.size())
    {
        Log::coreWarn("Color attachment Index out of range! (%i >= %i)", attachmentIdx, m_colorAttachments.size());
...
void GlFramebuffer::clearColorAttachment(const u32_t attachmentIdx)
{
    NB_PROFILE_DETAIL();
        ...
        case (Texture::FormatInternal::R32F):
        {
            ...
            Renderer::s_submitObject(
                [p_this, attachmentIdx]()
                {
                    ...
                    const std::array<f32_t, 4> clearColor = std::get<std::array<f32_t, 4>>(
        ...
        case (Texture::FormatInternal::R32I):
        {
            ...
            Renderer::s_submitObject(
                [p_this, attachmentIdx]()
                {
                    ...
                    const std::array<i32_t, 4> clearColor = std::get<std::array<i32_t, 4>>(
                        p_this->m_colorAttachments[attachmentIdx]->getSpec().clearColor);
        ...
        case (Texture::FormatInternal::R32UI):
        {
            ...
            Renderer::s_submitObject(
                [p_this, attachmentIdx]()
                {
                    ...
                    const std::array<u32_t, 4> clearColor = std::get<std::array<u32_t, 4>>(
                        p_this->m_colorAttachments[attachmentIdx]->getSpec().clearColor);
...
void GlFramebuffer::clearDepthAttachment()
{
    ...
    switch (m_spec.depthType)
    {
        ...
        default:
            NB_CORE_ASSERT(false, "Can't clear framebuffer depth attachment type %i", m_spec.depthType);
            break;
    }
}
...
{
    for (u32_t i = 0; i < m_colorAttachments.size(); i++)
    {
...
void GlFramebuffer::_construct()
{
    ...
    Renderer::s_submitObject(
        [p_this]() mutable
        {
            ...
            for (u32_t i = 0; i < p_this->m_spec.colorAttachments.size(); i++)
            {
            ...
            ////////////////////////////////////////////////////////////////////////////
            u32_t result = glCheckNamedFramebufferStatus(p_this->m_fbo, GL_FRAMEBUFFER);
            if (result != GL_FRAMEBUFFER_COMPLETE)
            {
                NB_CORE_ASSERT_STATIC(false, "Incomplete framebuffer! Error 0x%X", result);
...
u32_t GlFramebuffer::_textureTarget() const
{
